...
def featureExtraction(data2):
  ...
  data2['Shortening_Service'] = data2['url'].apply(lambda i: Shortining_Service(i))
  data2['having_At_Symbol'] = data2['url'].apply(lambda i: having_At_Symbol(i))
  data2['prefixSuffix'] = data2['url'].apply(lambda i: prefixSuffix(i))
  data2['double_slash_redirecting'] = data2['url'].apply(lambda i: double_slash_redirecting(i))
  data2['having_Sub_Domain'] = data2.apply(lambda x: having_Sub_Domain(x['url'], x['having_IP_Address']), axis=1)
  data2['httpSecure'] = data2['url'].apply(lambda i: httpSecure(i))
  data2['Domain_registration_length'] = data2['url'].apply(lambda i: Domain_registration_length(i))
...
def url_check(url):
  ...
  feature_name = ['Domain_in_Source', 'Length_of_Source', 'duplicated_HEAD', 'External_Load_Script']
  data[[i for i in feature_name]] = pd.DataFrame([i for i in feature], index=data.index)
